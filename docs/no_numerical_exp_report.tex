...
This report describes numerical tests for the non-optimizing algorithm, namely \gls{ARMA} and \gls{ARCH}.

These prediction models are being developed by the University of South Carolina. 
In this report, we discuss numerical experiments for testing the non-optimizing methods.
The numerical experiments will be designed for both the once through nuclear fuel 
...
The user defines prototypes to be deployed for fuel facilities, and the 
demand equation.  The reactor deployment causes the demand of fuel
...
The algorithm predicts the demand by looking at the previous demand and supply values.
...
adjustment of facilities occur prior to the next DRE.
