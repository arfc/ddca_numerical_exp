...
\node (Tick) [process, right of=Build] {Tick (agent)};
\node (DRE) [process, right of=Tick]{Dynamic Resource Exchange (kernel) };
\node (Tock) [process, right of=DRE]{Tock (agent)};
\node (Decom) [process, right of=Tock] {Decommission (kernel)};
