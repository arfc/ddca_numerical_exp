...
For each test scenario, there is one table that states the test scenario's input parameters and another table that states the exact analytical solution. In the latter table, there are columns specified for facility "decommissioning" and "dying". Decommissioning refers to when the \texttt{Institution} removes the facility. Whereas dying refers to when the facility's lifetime ends. 
...
\begin{table}[H]
	...
	\begin{tabular}{|P{6.5cm}|P{6.5cm}|}
		\hline
		\textbf{\shortstack{Acceptable total No. of Source \\Facilities Deployed + tolerance}} &\textbf{\shortstack{Acceptable total No. of Reactor \\Facilities Deployed + tolerance}} & \textbf{\shortstack{Acceptable total No. of \\ k \\Facilities Deployed + tolerance}}\\
		\hline
		6 $<$ x $<$ 7 & 2 $<$ x $<$ 3 & 4 $<$ x $<$ 5\\
		\hline
	\end{tabular}
