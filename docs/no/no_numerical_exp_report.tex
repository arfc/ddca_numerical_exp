...
Subsections \ref{subsection-user} to \ref{subsection-error} state the requirements for all 3 predictive algorithms. Expectations for the non-optimizing algorithm are different than for the deterministic-optimizing and stochastic-optimizing. Therefore, in subsections 3 to 5, the requirements unique to the non-optimizing algorithm are specified. 

\subsection{User Configuration}
\label{subsection-user}
The archetype must allow the user to define the following parameters: 
\begin{enumerate}
	...
	\item Type of algorithm: non-optimizing, deterministic optimizing or stochastic optimizing
\end{enumerate}

\subsection{Informing User about Input Errors}
\label{subsection-error}
The archetype must inform the user when the facilities specified do not produce appropriate commodities 
to form a supply chain to meet the demand of the commodity whose demand drives deployment.
